  ...
  useEffect(() => {
    ...
    const setupSSE = () => {
      try {
        ...
        const url = `/api/admin/disbursements/stream?since=${since}`;

        eventSource = new EventSource(url);

        eventSource.addEventListener('disbursement-updated', (event) => {
          const disbursement = JSON.parse(event.data);
          setDisbursements(prev => {
            const existing = prev.findIndex(d => d.id === disbursement.id);
            if (existing >= 0) {
              const updated = [...prev];
              updated[existing] = { ...updated[existing], ...disbursement };
              return updated;
            }
            return [disbursement, ...prev];
          });
          setTotalDisbursements(prev => prev + 1);
        });
