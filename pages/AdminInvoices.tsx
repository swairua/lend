...
import { Loader2, Plus, Eye, Trash2, Printer, ChevronLeft, ChevronRight, RefreshCw } from 'lucide-react';
...
export default function AdminInvoices() {
  ...
  const openDetail = async (inv: Invoice) => {
    try {
      const res = await adminApi.getInvoice(inv.id);
    ...
    setTimeout(() => {
      ...
      w.document.write(`
        ...
        <style>
          ...
          .badge { display: inline-block; padding: 2px 10px; border-radius: 4px; font-size: 12px; background: #e2e8f0; }
        </style></head><body>
        <div class="header">
          <div><h1>INVOICE</h1><p>${inv.invoice_number}</p></div>
        ...
        <p><strong>Invoice Date:</strong> ${formatDate(inv.invoice_date)} | <strong>Due Date:</strong> ${formatDate(inv.due_date)}</p>
        <table>
          <thead><tr><th>Description</th><th>Qty</th><th>Price</th><th>Tax</th><th>Amount</th></tr></thead>
          <tbody>
            ${(inv.items || []).map(it => `<tr><td>${it.description}</td><td>${it.quantity}</td><td>${formatKES(it.unit_price)}</td><td>${it.tax_rate}%</td><td>${formatKES(it.amount)}</td></tr>`).join('')}
          </tbody>
        </table>
        <div class="totals">
          <p><span>Subtotal:</span><span>${formatKES(inv.subtotal)}</span></p>
          <p><span>Tax:</span><span>${formatKES(inv.tax_total)}</span></p>
          ${inv.discount > 0 ? `<p><span>Discount:</span><span>-${formatKES(inv.discount)}</span></p>` : ''}
          <p style="font-weight:bold;font-size:16px;border-top:2px solid #333;padding-top:4px;"><span>Total:</span><span>${formatKES(inv.grand_total)}</span></p>
        </div>
        ${inv.notes ? `<div class="footer"><p><strong>Notes:</strong></p><p>${inv.notes}</p></div>` : ''}
        </body></html>
      `);
      w.document.close();
      w.print();
              ...
              <ResponsiveTableBody>
                {invoices.map(inv => (
                  <ResponsiveTableRow key={inv.id}>
                    ...
                    <ResponsiveTableCell>{inv.client_name}</ResponsiveTableCell>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  ...
                  {selected.status === 'sent' && (
                    <>
                      <Button size="sm" variant="outline" onClick={() => handleStatus(selected.id, 'paid')}>Mark Paid</Button>
                      <Button size="sm" variant="outline" onClick={() => handleStatus(selected.id, 'overdue')}>Mark Overdue</Button>
                    </>
                  )}
                  {selected.status !== 'cancelled' && (
                    <Button size="sm" variant="outline" onClick={() => handleStatus(selected.id, 'cancelled')}>Cancel</Button>
                  )}
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <p>Invoice: {formatDate(selected.invoice_date)}</p>
                  {selected.due_date && <p>Due: {formatDate(selected.due_date)}</p>}
                  {selected.quote_number && <p className="text-xs mt-1">From: {selected.quote_number}</p>}
                </div>
              </div>
