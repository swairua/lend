...
import { Loader2, Plus, Eye, Trash2, FileOutput, Printer, ChevronLeft, ChevronRight, RefreshCw, Search, Pencil } from 'lucide-react';
...
interface Quotation { id: number; quote_number: string; customer_id: number | null; customer_name?: string; client_name: string; client_email: string; client_phone: string; client_address: string; quote_date: string; expiry_date: string; subtotal: number; tax_total: number; discount: number; grand_total: number; notes: string; status: string; created_by_name?: string; created_at: string; items?: QuotationItem[]; }
...
const statusColors: Record<string, string> = { draft: 'bg-gray-100 text-gray-800', sent: 'bg-blue-100 text-blue-800', accepted: 'bg-green-100 text-green-800', rejected: 'bg-red-100 text-red-800', converted: 'bg-purple-100 text-purple-800' };
...
export default function AdminQuotations() {
  ...
  const load = async (p = 1) => {
    ...
    } catch (e: any) { toast.error(e.message || 'Failed to load quotations');
  ...
  const openCreate = () => {
    setEditTarget(null);
    setForm({ customer_id: null, client_name: '', client_email: '', client_phone: '', client_address: '', quote_date: new Date().toISOString().split('T')[0], expiry_date: '', notes: '', subtotal: 0, tax_total: 0, discount: 0, grand_total: 0, items: [{ ...emptyItem }] });
    setDialogOpen(true);
  };
  ...
  const openEdit = (q: Quotation) => {
    ...
    setForm({
      customer_id: q.customer_id, client_name: q.client_name, client_email: q.client_email || '',
      client_phone: q.client_phone || '', client_address: q.client_address || '',
      quote_date: q.quote_date, expiry_date: q.expiry_date || '', notes: q.notes || '',
      subtotal: q.subtotal, tax_total: q.tax_total, discount: q.discount, grand_total: q.grand_total,
  ...
  const openDetail = async (q: Quotation) => {
    try { const res = await adminApi.getQuotation(q.id); setSelected(res.data); setDetailOpen(true); } catch (e: any) { toast.error(e.message || 'Failed to load quotation'); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) {
        await adminApi.updateQuotation(editTarget.id, data);
        toast.success('Quotation updated');
      } else {
        await adminApi.createQuotation(data);
  ...
  const handleStatus = async (id: number, status: string) => {
    try { await adminApi.updateQuotationStatus(id, status); toast.success('Status updated'); load(page); if (selected?.id === id) setSelected(prev => prev ? { ...prev, status } : prev); } catch (e: any) { toast.error(e.message || 'Status update failed'); }
  };

  const handleConvert = async (id: number) => {
    try { const res = await adminApi.convertQuotation(id); toast.success(`Invoice ${res.data.invoice_number} created`); load(page); navigate('/admin/invoices'); } catch (e: any) { toast.error(e.message || 'Conversion failed'); }
  };
  ...
  const handlePrint = async (q: Quotation) => {
    try {
      ...
      const html = buildBrandedDocumentHtml({
        ...
        date_label: 'Quote Date', date_value: formatDate(quote.quote_date), due_label: 'Expiry Date', due_value: quote.expiry_date ? formatDate(quote.expiry_date) : undefined,
        items: (quote.items || []).map(it => ({ description: it.description, quantity: it.quantity, unit_price: it.unit_price, tax_rate: it.tax_rate, amount: it.amount })),
        subtotal: quote.subtotal, tax_total: quote.tax_total, discount: quote.discount, grand_total: quote.grand_total,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold">Quotations</h1><p className="text-gray-600">Create and manage client quotations</p></div>
      ...
      <Card>
        ...
        <CardContent>
          ...
          : quotations.length === 0 ? (<p className="text-center text-gray-500 py-8">No quotations yet.</p>)
          : (<ResponsiveTable>
              ...
              <ResponsiveTableBody>
                {quotations.map(q => (
                  <ResponsiveTableRow key={q.id}>
                    ...
                    <ResponsiveTableCell>
                      {q.client_name}
                      {q.customer_name && <span className="text-xs text-gray-400 ml-1">({q.customer_name})</span>}
                    </ResponsiveTableCell>
                    ...
                    <ResponsiveTableCell><Badge className={statusColors[q.status] || ''}>{q.status}</Badge></ResponsiveTableCell>
                    <ResponsiveTableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => handlePrint(q)}><Printer className="w-3 h-3" /></Button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editTarget ? 'Edit Quotation' : 'New Quotation'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><Label>Quote Date</Label><Input type="date" value={form.quote_date} onChange={e => setForm({...form, quote_date: e.target.value})} /></div>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button><Button onClick={handleSave}>{editTarget ? 'Update Quotation' : 'Create Quotation'}</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{selected?.quote_number || 'Quotation'}</DialogTitle></DialogHeader>
          {selected && (<div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {(selected.status === 'accepted' || selected.status === 'draft') && <Button size="sm" onClick={() => handleConvert(selected.id)}><FileOutput className="w-3 h-3 mr-1" />Convert to Invoice</Button>}
                <Button size="sm" variant="outline" onClick={() => handlePrint(selected)}><Printer className="w-3 h-3 mr-1" />Print</Button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right"><p className="text-xs text-gray-500">Dates</p><p>Quote: {formatDate(selected.quote_date)}</p>{selected.expiry_date && <p>Expiry: {formatDate(selected.expiry_date)}</p>}</div>
            </div>
