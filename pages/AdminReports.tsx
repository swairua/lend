  ...
  const loadReport = async () => {
    ...
    try {
      if (activeReport === 'portfolio' || activeReport === 'financial') {
        ...
        const totalDisbursed = response.data.loans
          .filter((l: any) => l.status === 'active' || l.status === 'disbursed')
          .reduce((sum: number, l: any) => sum + Number(l.principal_amount), 0);
        const totalOutstanding = response.data.loans
          .reduce((sum: number, l: any) => sum + Number(l.balance || 0), 0);
        const totalCollected = response.data.loans
        ...
        setStats({
          ...
          pending_count: response.data.loans.filter((l: any) => l.status === 'pending').length,
          active_count: response.data.loans.filter((l: any) => l.status === 'active' || l.status === 'disbursed').length,
          completed_count: response.data.loans.filter((l: any) => l.status === 'completed').length,
        });
      }
