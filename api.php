...
try {
    ...
    if ($resource === 'public') {
        if (strpos($uri, 'public/categories') !== false) {
            echo json_encode(['success' => true, 'data' => all("SELECT * FROM loan_categories WHERE is_active = 1 ORDER BY name")]);
            exit;
        }
        if (preg_match('#public/products/(\d+)#', $uri, $m)) {
            echo json_encode(['success' => true, 'data' => one("SELECT * FROM loan_products WHERE id = ?", [$m[1]])]);
            exit;
        }
        if (strpos($uri, 'public/products') !== false) {
            ...
            echo json_encode(['success' => true, 'data' => all($sql, $params)]);
            exit;
        }
    ...
    if ($resource === 'borrower') {
        ...
        if (preg_match('#borrower/loans/(\d+)#', $uri, $m)) {
            ...
            if (!$loan) { http_response_code(404); echo json_encode(['success' => false, 'error' => 'Not found']); exit; }
        ...
        if (strpos($uri, 'borrower/loans') !== false) {
            if ($method === 'POST') {
                ...
                if (!$product) { echo json_encode(['success' => false, 'error' => 'Invalid product']); exit; }
                $amount = floatval($d['amount'] ?? 0);
                $term   = intval($d['term_months'] ?? 1);
                if ($amount < $product['min_amount'] || $amount > $product['max_amount']) {
                    echo json_encode(['success' => false, 'error' => 'Amount out of range']); exit;
                }
                if ($term < $product['min_term_months'] || $term > $product['max_term_months']) {
                    echo json_encode(['success' => false, 'error' => 'Term out of range']); exit;
                }
                $interest = ($amount * floatval($product['interest_rate']) / 100) * ($term / 12);
                $procFee  = $amount * floatval($product['processing_fee_percent']) / 100;
                $assetFee = floatval($product['asset_transfer_fee']);
    ...
    if ($resource === 'messages') {
        ...
        if ($method === 'GET' && preg_match('#messages/(\d+)$#', $uri, $m)) {
            ...
            echo json_encode(['success' => true, 'data' => $msg]); exit;
        }
        if ($method === 'POST') {
            $d = input();
            q("INSERT INTO messages (sender_id, recipient_id, loan_id, subject, message, type)
    ...
    if ($resource === 'admin') {
        ...
        if ($method === 'POST' && preg_match('#admin/loans/(\d+)/approve$#', $uri, $m)) {
            ...
            if (!$loan || $loan['status'] !== 'pending') {
                echo json_encode(['success' => false, 'error' => 'Loan is not pending']); exit;
            }
        ...
        if ($method === 'POST' && preg_match('#admin/loans/(\d+)/disburse$#', $uri, $m)) {
            ...
            if (!$loan || $loan['status'] !== 'approved') {
                echo json_encode(['success' => false, 'error' => 'Loan must be approved first']); exit;
            }
            $d = input();
        ...
        if (strpos($uri, 'admin/categories') !== false) {
            if ($method === 'POST') {
                ...
                echo json_encode(['success' => true, 'data' => ['id' => pdo()->lastInsertId()]]); exit;
            }
